...
const MyPostedJobs = () => {
  ...
  const getData = async () => {
    try {
      const response = await fetch(`${import.meta.env.VITE_API_URL}/jobs/${user?.email}`);
      if (!response.ok) {
  ...
  return (
    ...
    <Helmet><title>My Posted Jobs</title></Helmet>
                    ...
                    <tr key={job._id}>
                      ...
                      <td className='px-4 py-4 text-sm whitespace-nowrap'>
                        <div className='flex items-center gap-x-6'>
                          <button onClick={() => handleDelete(job._id)}   className='  transition-colors duration-200   hover:text-red-500 focus:outline-none'  >
                            <svg
                          ...
                          <Link to={`/update/${job._id}`} className='  transition-colors duration-200   hover:text-yellow-500 focus:outline-none'  >
                            <svg
                              xmlns='http://www.w3.org/2000/svg'
                              fill='none'
                              viewBox='0 0 24 24'
    ...
    </section>:<p className='text-6xl font-bold text-center text-red-700'>You Have No Data Pushed on the Form</p>
      }
    </div>
  )
}
